...
const json = resolve("playstation.us.json");

export default async function (browser: Browser) {
  const { stop } = spinner("[playstation.us] Fetching game urls...");

  const page = await browser.newPage();
  await page.goto("https://store.playstation.com/en-us/collections");
  ...
  const bar = new SingleBar(
    { format: "[playstation.us] [{bar}] {percentage}% ({value}/{total}) | ETA {eta_formatted}", etaBuffer: 100 },
